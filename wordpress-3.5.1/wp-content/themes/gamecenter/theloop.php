	...
	while (have_posts()) : the_post(); ?>
		<div class='one-post'>
			...
			if (!is_page()) {?><p class='post-meta'>
				 ...
				 Releases with <span class='post-comments'><?php comments_popup_link( $SMTheme->_( 'noresponses' ), $SMTheme->_( 'oneresponse' ), $SMTheme->_( 'multiresponse' ) ) ?></span>
				
				Posted in <?php the_category(', '); 				
				if(comments_open( get_the_ID() ))  {
                    ?>
				  On <span class='post-date'><?php echo get_the_date(); ?></span>
				<?php } 
